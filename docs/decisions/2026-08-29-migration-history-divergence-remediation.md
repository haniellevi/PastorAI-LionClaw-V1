...
somente o preflight de identidade de DEV, em leitura e com autorização
separada. Não autoriza captura, materialização de artefato, DML, reconciliação
de ledger, corte de época, alteração ou execução do runner, migration,
backfill, deploy, flag ou runtime. PROD está explicitamente fora.
