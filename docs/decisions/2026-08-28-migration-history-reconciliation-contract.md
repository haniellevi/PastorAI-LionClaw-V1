...
Posteriormente, esse caminho foi supersedido pelos diagnósticos de fase e pelo
probe transport-only executados sob autorizações humanas nominais próprias. O
identificador permanece somente como registro histórico e não é gate corrente
nem próximo hoje.
...
O executor emite somente evidência v1 sanitizada e bloqueada. Mesma conexão/PID
significa duas transações e dois snapshots `REPEATABLE READ READ ONLY`
separados, não uma única fotografia PostgreSQL. Nenhuma decisão do contrato,
revisão v3, migration ou autorização operacional foi completada.
