...
`SEPARATE_NOMINAL_DEV_READ_ONLY_PREFLIGHT_AUTHORIZATION` é o próximo gate único.
Ele exige uma autorização humana nova e exclusiva para uma invocação
process-only (`PROCESS_INVOCATION_ONLY`) do preflight de identidade de DEV. Não
autoriza captura, materialização, DML,
