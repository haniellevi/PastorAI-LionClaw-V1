...
`reset_tudo.py` é dry-run por padrão, exige URL PostgreSQL explícita, role com
`rolsuper` ou `rolbypassrls`, backup atestado e confirmação interativa do host.
O reset mantém schema, migrations, planos, admins e audit de plataforma. Nenhum
banco compartilhado, backup ou provedor foi acionado nesta fatia.

No reset, toda tabela ordinária ou particionada `public.e4b_*` é enumerada
antes de DML. Ausente ou vazia permite seguir; qualquer contagem positiva
produz `BLOCKED_E4B_POPULATED`, nomes e contagens sem PII, código `3` e rollback.
A execução repete a checagem sob locks depois da confirmação do host. A
limitação evita contornar retenção imutável. A migration segue o processo
simples do MVP por `migrate.py`. Contrato e operação futura estão em
`docs/decisions/2026-09-25-tenant-deletion.md` e
`docs/ops/TENANT-DELETION-RESET-RUNBOOK.md`. O próximo gate humano único desta
fatia é Raniel autorizar o merge por número da PR B. No candidato local baseado
em `34d3a7282b3c712b0e5f7bc636bb2a20b72db929`, `./test-local.sh` passou com
4.970 testes backend, 854 frontend e typecheck; RLS em PostgreSQL 17 sintético
passou com 288 testes, sem falha ou skip. Isso não prova banco compartilhado,
provedor ou execução real.
...
| LangGraph | `IMPLEMENTADO STATELESS / D2B1 INTEGRADA / PREPARAÇÃO D3 INTEGRADA / FUNDAÇÃO D3 REPLAY-ONLY E CELL-REPORT CANDIDATAS OFFLINE / SESSÃO DEDICADA INTEGRADA EM MAIN E INATIVA` | Projetar projeções/writers mínimos e, depois, provisionar grants/views limitados e ligar o staging transacional e o coordenador fail-closed a caller, consentimento, `AgentConfig`, commit, despacho e receipts duráveis |
...
| Relatório por WhatsApp | `PARCIAL / STAGING TRANSACIONAL OFFLINE CANDIDATO` | Conectar o coordenador e a UoW candidatos ao agente, após integração com o resolvedor de reunião, consentimento, `AgentConfig`, commit externo, despacho e comprovante pós-commit |
...
turno. Essa contenção reduz o risco de replay acidental, mas não preserva
...
O lote local adiciona `AgentTurnIdentity`, `AgentEffectIntent`, plano estrutural
e um adaptador replay-only da saída fechada do grafo. A identidade confiável
está atrás de flag desligada; plano e receipts continuam valores puros sem
persistência. O adaptador apenas reconhece replay já terminal quando plano e
receipts exatos são fornecidos por uma futura fonte confiável. Ele não executa
a primeira tentativa, não possui consumer de runtime e não torna o grafo
...
estava `false`, e não houve deploy manual ou do backend, wiring, ativação ou canário. Painel do
...
- o preflight runtime comprovou identidade, owner e ACL; o caminho
  `M06_MIGRATION_DATABASE_URL` e `public.schema_migrations` estavam ausentes em
  PROD no preflight histórico de 2026-08-28; o estado vivo atual não foi
  revalidado, e o `FORCE RLS` da tabela futura não foi comprovado. O
...
Ele usa snapshot privado e entradas por descritores, mantém uma conexão/PID e
executa duas transações e dois snapshots `REPEATABLE READ READ ONLY` separados
para identidade e captura. O artefato v1 continua sanitizado e bloqueado.
...
`psycopg2-binary==2.9.12` e libpq 17. As linhas
`AUTHORIZATION_TRUST_REQUIREMENT=EXTERNAL_NOMINAL_GATE_AUTHENTICATION_REQUIRED`
e `RUNTIME_TRUST_REQUIREMENT=EXTERNALLY_PINNED_RUNTIME_REQUIRED` declaram
requisitos externos ainda ausentes e não substituem um launcher confiável
antes dos descritores de segredo. O campo `next_gate` congelado no pacote v3
...
A preparação D3 integrada mantém esse estado inativo. `AgentTurnInput` e
`AgentTurnOutput` delimitam a entrada e a saída, enquanto `AgentTurnEffects` é
reinicializado por turno, substituído sem reducer acumulativo e mantido em
`UntrackedValue`. Ele representa somente intenções efêmeras entregues ao runtime
existente. Não há saver, migration, memória, resumo, recuperação ou retomada.
