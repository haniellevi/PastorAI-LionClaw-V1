...
| Triagem semântica Jev (TypeSafe) | `MÓDULO ENTREGUE / NÃO INTEGRADO AO TURNO / EGRESSO ADMIN VIVO ATRÁS DE ALLOW_REAL_SENDS` | `backend/app/services/semantic_triage.py`, `GET /admin/jev` e `POST /admin/jev/teste` com teste sintético auditado e limitado, botão Jev no Admin Master e ADR `docs/decisions/2026-09-23-jev-shadow-triage.md` | Ligação ao runtime depende de revisão do gate D3 e de desenho fora da transação; nenhuma igreja pode ser listada sem DPA, base legal para dado sensível em claro e revisão independente |
...
| Relatório pelo WhatsApp | `PARCIAL / STAGING TRANSACIONAL OFFLINE CANDIDATO` | parser, snapshot, workflow, envelope, serviço ORM, UoW que agrupa relatório, auditoria e `Message` `ia_pendente` com flush sem commit, e coordenador offline fail-closed que relê o inbound bloqueado, exige alvo opaco, aceita somente texto puro sem metadados de mídia e mantém ciclo de proposta ligado a turno e transação | Não existe caller integrado no agente, runtime, worker, webhook, router humano ou `turn_plan_adapter`; também faltam integração com o resolvedor confiável de reunião, fonte aprovada de consentimento `tarefas_operacionais`, `AgentConfig`, commit, outbox, receipt global, resposta pós-commit e idempotência operacional comprovada. `transcribe_audio` existe em `backend/app/services/llm.py` com testes dedicados, mas sem caller de produção no WhatsApp nem prova operacional; o merge não alterou nem autorizou `AgentConfig.ativo` |
...
| Exclusão de tenant e reset administrativo | `CANDIDATA LOCAL / REVISÃO E MERGE` | candidato local grava manifesto externo em audit, preserva platform admin destacado, bloqueia E4B imutável e oferece reset transacional com dry-run; o reset enumera `public.e4b_*` e bloqueia linhas presentes | A migration de `app_users.igreja_id` anulável segue o processo simples por `migrate.py`; revisar o SHA e Raniel autorizar o merge por número da PR B. Execução de banco ou provedor continua fora desta fatia |
