...
| Triagem semântica Jev (TypeSafe) | `MÓDULO ENTREGUE / NÃO INTEGRADO AO TURNO / EGRESSO ADMIN VIVO ATRÁS DE ALLOW_REAL_SENDS` | `backend/app/services/semantic_triage.py`, `GET /admin/jev` e `POST /admin/jev/teste` com teste sintético auditado e limitado, botão Jev no Admin Master e ADR `docs/decisions/2026-09-23-jev-shadow-triage.md` | Gate D3 removido na Fase 0. Avaliação offline J0 em `backend/scripts/jev_eval.py` (corpus sintético pt-BR; sem chave mede só as regras). Ligação ao runtime (J1) só depois do envio, fora da transação do turno, e só com J0 = GO, DPA, termo LGPD citando IA e processador estrangeiro e base legal para dado sensível em claro |
