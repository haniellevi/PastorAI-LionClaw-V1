...
| LangGraph | `IMPLEMENTADO STATELESS / PREPARAÇÃO D3 INTEGRADA / FUNDAÇÃO D3 REPLAY-ONLY E CELL-REPORT CANDIDATAS OFFLINE` | Grafo único e fallback determinístico, sem checkpoint durável; identidade confiável está atrás de flag desligada, enquanto plano, receipts, adaptador replay-only, snapshot agregado e workflow de confirmação permanecem sem bridge de execução, persistência ou consumer de runtime |
...
| Relatório de célula por WhatsApp | `PARCIAL / FUNDAÇÃO OFFLINE CANDIDATA` | Snapshot agregado e workflow puro de coleta, revisão e confirmação existem no lote local, mas não há bridge, gravação canônica nem envio |
