...
| LangGraph | `IMPLEMENTADO STATELESS / PREPARAÇÃO D3 INTEGRADA / FUNDAÇÃO D3 REPLAY-ONLY E CELL-REPORT CANDIDATAS OFFLINE` | Grafo único e fallback determinístico, sem checkpoint durável; a fronteira ORM do relatório existe somente como candidata local, sem caller, consentimento, commit/outbox ou consumer de runtime |
...
| Relatório de célula por WhatsApp | `PARCIAL / FRONTEIRA TRANSACIONAL OFFLINE CANDIDATA` | Snapshot, workflow, proposta pendente e serviço ORM existem no lote local; nenhum caller os conecta ao agente ou ao writer web, e não há consentimento, commit/outbox, envio ou efeito vivo |
