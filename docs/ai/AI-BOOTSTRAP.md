...
| LangGraph | `IMPLEMENTADO STATELESS / PREPARAÇÃO D3 INTEGRADA / FUNDAÇÃO D3 REPLAY-ONLY E CELL-REPORT CANDIDATAS OFFLINE` | Grafo único e fallback determinístico, sem checkpoint durável; a UoW e o coordenador fail-closed do relatório existem somente como candidatos locais, sem caller, consentimento, `AgentConfig`, commit, send ou consumer de runtime |
...
| Relatório de célula por WhatsApp | `PARCIAL / STAGING TRANSACIONAL OFFLINE CANDIDATO` | Snapshot, workflow, proposta, serviço ORM, locks nos writers web, UoW e coordenador fail-closed existem localmente; o coordenador aceita somente texto puro e recusa mídia/caption; não há caller, resolvedor de reunião, consentimento aprovado, `AgentConfig`, commit, send ou efeito vivo |
