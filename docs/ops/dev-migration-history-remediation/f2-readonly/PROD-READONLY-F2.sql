...
-- F2 PROD catalog observation. Raniel executes this file manually only after
...
\else
\qecho F2_ABORT_PROD_LEDGER_SHAPE_DRIFT
ROLLBACK;
\quit
\endif
...
SELECT
  'PUBLIC_LEDGER_FORM' AS record_type,
  'ABSENT_ACCEPTED' AS state;
...
SELECT
  ...
  'PUBLIC_ABSENT_AND_NATIVE_6_OR_ABORT' AS ledger_shape_assertion;

ROLLBACK;
\qecho F2_PROD_FINAL_RECEIPT=ROLLBACK_COMPLETED_F2_PROD
