...
def test_migration_postgres_real(rls_engine, rls_seeded: dict[str, str]) -> None:  # noqa: F811
    ...
    with rls_engine.begin() as conn:
        # pessoa propria do teste (isolada da seedada pela fixture de sessao,
        # que persiste no container entre execucoes) — representa um registro
        # "existente" ANTES da migration/arquivamento rodarem. Removida no
        # finally: o schema/seed de conftest_rls e compartilhado com outros
        # testes rls_integration na mesma sessao/container.
    ...
    try:
        ...
        with rls_engine.begin() as conn:
            conn.execute(text("delete from pessoas where id = :id"), {"id": pessoa_id})
